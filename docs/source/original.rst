...
-  description: a string with the complete description for the
       announcement (basically the body)
...
-  tags: (Tags[]): Returns an array of tag objects applied to the
       announcement

-  events: (Events[]): Returns an array of event objects applied to the
       announcement

-  deadlines: (Deadlines[]): Returns an array of the deadlines that are
...
-  visible: single bit: whether the tag is visible when the announcement
       is displayed

-  minUserLevelAssign: single digit int: the minimum level that the user
       must be to be able to assign or approve the assignment of this
       tag

-  minUserLevelRequest: single digit int: the minimum level that the
       user must be to be to request that his/her announcement be
       assigned this tag

-  parentId (alias: parentID) if the tag is a child of another tag, the
       ID of the parent is put here, likely null for the moment
...
-  announcementID the announcement ID of which the event is a child
   ...
   -  announcement: the announcement parent with the id of the above
          announcementId
...
-  response: announcement []: an array of announcement objects which are
       the actual announcements that are returned
...
-  response: event []: an array of event objects which are the actual
...
-  response: tag []: an array of tag objects which are the actual tags
       that are returned
...
if a tagId is given, a list of announcement objects with that tag will
be returned in the array

if start and end dates are added, only announcements visible between the
two dates (exclusive) are returned
...
For updating events or tags, use the POST methods for
/announcements/:id/events or /announcements/:id/tags. These often have
an addXxxx/removeXxxx property to pass in that is an array of tag/event
ids.
